...
DOCS = [
    ...
    ("updates", "Updates", "Release discovery, verification primitives, and the unfinished production path.", "phase-11-self-update-patch-system"),
...
def features():
  ...
  <section class="light"><div class="wrap"><div class="cards">{card('Chromium browsing', 'Fast modern web browsing powered by the Android System WebView runtime. The installed WebView provider determines the exact Chromium version.', 'Implemented')}{card('Profiles', 'Profiles separate supported cookies, storage, permissions, history, sessions, identity settings, and network configuration.', 'Implemented')}{card('Network and proxy', 'Choose a direct route or configure HTTP, HTTPS, SOCKS4, or SOCKS5 behavior where the device runtime supports it.', 'Implemented')}{card('Privacy and security', 'JavaScript, cookies, pop-ups, Safe Browsing, site permissions, HTTPS state, download checks, and clear-data controls.', 'Implemented')}{card('Ad and tracker blocking', 'Profile-aware local blocking with site exceptions and request statistics. Blocking is not a promise that every ad or tracker is stopped.', 'Implemented')}{card('Performance', 'Resource policy coordinates profile and WebView lifecycle states to keep multiple browsing contexts manageable.', 'Implemented')}{card('Browser features', 'Downloads, bookmarks, history, private tabs, find-in-page, zoom, full-screen, file selection, and external-link intents.', 'Implemented')}{card('Self-updates', 'Typed manifests, checksum/signature primitives, and release discovery are present. Production artifact delivery is still unfinished.', 'In progress')}{card('Default browser', 'Android browser-role controls and HTTP/HTTPS intents respect Android system choice and user consent.', 'Implemented')}</div></div></section>
...
def changelog():
  ...
  <section class="light"><div class="wrap split"><div><div class="kicker">Current baseline</div><h2>Foundation is in place.</h2></div><div class="prose"><h3>Added</h3><ul><li>Chromium-backed Android browsing shell with tabs, history, bookmarks, downloads, find-in-page, zoom, and full-screen support.</li><li>Profile management, profile-scoped browser state, network configuration, privacy controls, ad/tracker blocking, and lifecycle management.</li><li>Android browser intents, default-browser role controls, permissions, update discovery, and legal screens.</li></ul><h3>Improved</h3><ul><li>Device identity profiles and WebView compatibility behavior are applied through the existing browser architecture.</li><li>Website and documentation are kept honest about public releases and unfinished production infrastructure.</li></ul><h3>Fixed</h3><ul><li>Current release state no longer implies an APK is available when the official repository has no published stable asset.</li></ul></div></div></section>
...
DOC_CONTENT = {
    ...
    "updates": ("Release discovery is not silent installation", "The current app reads official release metadata and lets users review a release before opening it. Typed manifest, checksum, signature, patch, rollback, and recovery primitives exist in the project, but production release infrastructure is not yet configured.", ["No public APK is shown unless a real published release asset exists.", "Private signing keys, keystores, and update secrets must remain outside the repository.", "The intended production path includes verification, atomic apply, rollback, and startup validation."]),
    ...
    "developer": ("Build in public", "The repository contains the Android app, phase blueprints, legal source, website generator, Pages workflow, and tests. The static site is generated by a small standard-library Python script and deployed as a GitHub Pages artifact.", ["Run the website generator with `python3 website/build.py`.", "Run Android checks through the repository Gradle wrapper and the configured Android toolchain.", "Never commit API tokens, signing keys, keystores, passwords, or private release infrastructure."]),
}
...
def security():
  ...
  <section class="dark"><div class="wrap split"><div><div class="eyebrow">Production boundary</div><h2>Not configured yet.</h2></div><div class="prose"><p>Production signing keys, release endpoints, delta application, staged rollout infrastructure, and a public signed APK are not represented as complete when they are not actually available. The repository's update blueprint describes the intended path without exposing private keys or claiming deployment.</p><div class="actions"><a class="button primary" href="{url('releases')}">Check release status</a><a class="button ghost" href="{github_file('PHASE_AUDIT.md')}">Implementation audit</a></div></div></div></section>
</main>'''
