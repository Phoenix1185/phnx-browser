...
def security():
  ...
  <section class="dark"><div class="wrap split"><div><div class="eyebrow">Production boundary</div><h2>Published releases.<br>Visible status.</h2></div><div class="prose"><p>Protected Android signing and separate update-manifest signing are wired into the tag-based release workflow. Published stable releases expose their real APK, AAB, manifest, and checksums through GitHub Releases. Delta application, staged rollout, background policy, and automatic installation remain deliberately unclaimed.</p><div class="actions"><a class="button primary" href="{url('releases')}">Check release status</a><a class="button ghost" href="{github_file('PHASE_AUDIT.md')}">Implementation audit</a></div></div></div></section>
</main>'''
