...
- Phase 9: the normal CI build remains unsigned by design; the tag-based signed-release workflow now uses repository secrets for Android and separate update signing, but migration/upgrade coverage, network-security hardening, exact generated license notices, rollback documentation, and repository security controls remain.
...
- Phase 11: the dedicated update public key and signed release manifest are wired, while key rotation, delta/full-APK download fallback, atomic package apply, staged rollout, background policy, and a production manifest download service remain.
